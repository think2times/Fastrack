...
(define (install-integer-package)
  ...
  (put 'atan-generic '(integer integer)
       (lambda (x y) (atan (raise (tag x)) (raise (tag y))))) ; 把整数形式升级为有理数
  (put 'sqrt-generic '(integer)
       (lambda (x) (sqrt (raise (tag x)))))   ; 把整数形式升级为有理数
...
(define (install-polar-package)
  ...
  (define (make-from-real-imag x y)
    (cons (sqrt-generic (add (square x) (square y)))
          (atan-generic y x)))
...
(define c1 (make-complex-from-real-imag rat1 n1))
(define c2 (make-complex-from-real-imag rat2 rat1))
(define c3 (make-complex-from-mag-ang r2 n2))
(define c4 (make-complex-from-mag-ang n2 r2))
